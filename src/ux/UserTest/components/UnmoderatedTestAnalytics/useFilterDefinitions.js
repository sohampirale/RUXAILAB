...
export function useFilterDefinitions({
  ...
  const filterDefinitions = computed(() => {
    ...
    const post = testStructure.value?.postTest || []

    const preVars = pre.map((q, idx) => {
      ...
      const baseItems = isCategorical
        ? options.map((o) => ({ title: o, value: o }))
        : []
      ...
      return {
        ...
        type: 'pre',
        title:
          q.title || q.question || t('analytics.question', { number: idx + 1 }),
        options,
        isCategorical,
        items: baseItems,
      }
    ...
    const postVars = post.map((q, idx) => {
      const answerValueSet = new Set()
      Object.values(answers.value).forEach((s) => {
        const a = s.postTestAnswer?.[idx]?.answer
        if (a !== undefined && a !== null && a !== '') answerValueSet.add(a)
      })
